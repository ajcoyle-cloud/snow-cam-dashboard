...
// (/api/om/v1/forecast?…) instead of hitting api.open-meteo.com from the
// browser, which buys three things:
//
...
//
// The upstream host is fixed to api.open-meteo.com and can never be pointed
// elsewhere (no open-proxy / SSRF surface): the catch-all only contributes the
// path + query, and the origin is this constant.
//
// vercel.json needs no entry — api/om/[...path].js is a catch-all API route, so
// /api/om/v1/forecast?… lands here with req.query.path = ['v1','forecast'].
// Local dev has no Vercel runtime, so vite.config.js proxies /api/om to
// Open-Meteo directly (uncached) to keep parity.
...
export default async function handler(req, res) {
  // req.url is /api/om/<path>?<query>; forward exactly <path>?<query> upstream.
  // Reconstructing from req.url (rather than rebuilding from req.query) keeps
  // the query string byte-for-byte, which matters for the comma-separated
  // hourly=… lists and multi-value latitude=…,… the isobar grid sends.
  const suffix = req.url.replace(/^\/api\/om/, '');
  if (!suffix.startsWith('/v1/')) {
    res.setHeader('Cache-Control', 'no-store');
    res.status(400).json({ error: 'bad_path', detail: 'Only /v1/* Open-Meteo paths are proxied.' });
    return;
  }

  try {
    const upstream = await fetch(UPSTREAM + suffix, { headers: { Accept: 'application/json' } });
