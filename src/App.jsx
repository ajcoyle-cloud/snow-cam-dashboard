...
const NORTH_ISLAND = [
  { name: 'Happy Valley', url: 'https://webcams.whakapapa.com/hvfromskywaka/latest.jpg', location: 'Whakapapa', elevation: 1620 },
...
function SnowfallForecast({ resort, setResort, onOpenCompare }) {
  ...
  // Which voice engine actually spoke the last summary — surfaced in the UI so
  // a silent fallback to the browser voice is visible/debuggable rather than
  // just sounding different with no explanation.
  const [voiceEngine, setVoiceEngine] = useState(null) // null | 'elevenlabs' | 'browser'
  const [voiceEngineDetail, setVoiceEngineDetail] = useState(null)
  ...
  useEffect(() => {
    const fetchForecast = async () => {
      ...
      try {
        ...
        const ecmwfBaseUrl = `/api/om?latitude=${r.lat}&longitude=${r.lon}&elevation=${r.baseElev}&hourly=temperature_2m,wet_bulb_temperature_2m,precipitation,precipitation_probability,snowfall,weather_code&models=ecmwf_ifs025&temperature_unit=celsius&timezone=${r.timezone}&forecast_days=16`
        ...
        const aifsSummitUrl = `/api/om?latitude=${r.lat}&longitude=${r.lon}&elevation=${r.summitElev}&hourly=temperature_2m,wet_bulb_temperature_2m,precipitation,precipitation_probability,snowfall,weather_code&models=ecmwf_aifs025_single&temperature_unit=celsius&wind_speed_unit=kmh&timezone=${r.timezone}&forecast_days=16`
        const aifsBaseUrl = `/api/om?latitude=${r.lat}&longitude=${r.lon}&elevation=${r.baseElev}&hourly=temperature_2m,wet_bulb_temperature_2m,precipitation,precipitation_probability,snowfall,weather_code&models=ecmwf_aifs025_single&temperature_unit=celsius&timezone=${r.timezone}&forecast_days=16`
        // UK Met Office seamless (global 10km, falls back from the UK-only 2km model outside Britain) — only a 7-day horizon.
        const ukmoSummitUrl = `/api/om?latitude=${r.lat}&longitude=${r.lon}&elevation=${r.summitElev}&hourly=temperature_2m,wet_bulb_temperature_2m,precipitation,precipitation_probability,snowfall,weather_code,windspeed_700hPa,winddirection_700hPa,windspeed_850hPa,winddirection_850hPa&models=ukmo_seamless&temperature_unit=celsius&wind_speed_unit=kmh&timezone=${r.timezone}&forecast_days=16`
        const ukmoBaseUrl = `/api/om?latitude=${r.lat}&longitude=${r.lon}&elevation=${r.baseElev}&hourly=temperature_2m,wet_bulb_temperature_2m,precipitation,precipitation_probability,snowfall,weather_code&models=ukmo_seamless&temperature_unit=celsius&timezone=${r.timezone}&forecast_days=16`
  ...
  // quota exhausted, network error) resolves { ok: false, reason } so the
  // caller falls back to the browser's built-in voice — reason is surfaced
  // in the UI (see voiceEngine/voiceEngineDetail) and logged to the console
  // so a failure is debuggable instead of silently invisible.
  const speakViaElevenLabs = async (text) => {
    if (typeof window === 'undefined' || typeof Audio === 'undefined') {
      return { ok: false, reason: 'Audio not supported in this environment.' }
    }
    try {
      ...
      if (!res.ok) {
        let reason = `ElevenLabs endpoint returned ${res.status}`
        try {
          const errJson = await res.json()
          reason = errJson?.detail || errJson?.error || reason
        } catch (e) {}
        console.warn('[ai-summary] ElevenLabs TTS failed, falling back to browser voice:', reason)
        return { ok: false, reason }
      }
      const blob = await res.blob()
      if (!blob || blob.size === 0) {
        console.warn('[ai-summary] ElevenLabs returned an empty audio response, falling back.')
        return { ok: false, reason: 'ElevenLabs returned empty audio.' }
      }
      ...
      console.warn('[ai-summary] ElevenLabs TTS threw, falling back to browser voice:', reason)
      return { ok: false, reason }
    }
  }
  ...
  // automatic fallback — see speakViaElevenLabs()'s doc comment. Tracks which
  // engine actually spoke (voiceEngine) and, on fallback, why (voiceEngineDetail)
  // so that's debuggable/visible instead of indistinguishable in the UI.
  const speak = async (rawText) => {
    stopSpeaking()
    const text = makeSpeakable(rawText)
    setTtsLoading(true)
    const result = await speakViaElevenLabs(text)
    setTtsLoading(false)
    if (result.ok) {
      ...
      setVoiceEngineDetail(result.reason || null)
    ...
    <div className="forecast-container" ref={containerRef}>
      ...
      {(aiLoading || ttsLoading || aiError || aiSummary) && (
        <div className="ai-summary">
          ...
          {voiceEngine === 'elevenlabs' && <div className="ai-summary-voice">Voice: ElevenLabs</div>}
          {voiceEngine === 'browser' && (
            <div className="ai-summary-voice ai-summary-voice-fallback">
              Voice: browser fallback{voiceEngineDetail ? ` — ${voiceEngineDetail}` : ''}
            </div>
          )}
        </div>
      )}
    ...
    Object.entries(RESORTS).forEach(async ([key, r]) => {
      ...
      try {
        const modelUrl = (model, elev) =>
          `/api/om?latitude=${r.lat}&longitude=${r.lon}&elevation=${elev}&hourly=temperature_2m,precipitation,snowfall&models=${model}&temperature_unit=celsius&timezone=${r.timezone}&forecast_days=${COMPARE_FORECAST_DAYS}`
